				...
				<tr>
					<td width="93%" class="ct_ttl01">					
						<c:if test="${menu=='manage' }">
							상품 관리    ${search.searchCondition}
						</c:if>		
						<c:if test="${menu!='manage' }">									
							상품목록조회   ${search.searchCondition}
						</c:if>		
					</td>
				</tr>
			</table>
	...
	<c:forEach var="product" items="${list }">
		<c:set var="i" value="${i+1 }" />
			<tr class="ct_list_pop">
				...
				<td align="left">
					<c:choose>
						<c:when test="${product.proTranCode == null || product.proTranCode == 999}">
							판매중
						</c:when>
						<c:when test="${product.proTranCode == '001'}">
							<c:if test="${menu =='manage'}">
								구매완료		
								<a href="/purchase/updateTranCodeByProd?tranNo=${product.tranNo}&tranCode=002">배송하기</a>								
							</c:if>
							<c:if test="${menu !='manage'}">
								재고없음
							</c:if>
						</c:when>
						<c:when test="${product.proTranCode == '002'}">
							배송중
						</c:when>
						<c:when test="${product.proTranCode == '003'}">
							배송완료
						</c:when>	
						<c:otherwise>
							관리자확인
